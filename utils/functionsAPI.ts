...
export async function initializePayFastTopUp(
  userId: string,
  amount: number,
  opts?: { email?: string; itemName?: string; itemDescription?: string }
) {
  const payload: any = {
    amount: Number(amount).toFixed(2),
    itemName: opts?.itemName || 'Wallet Top-up',
    itemDescription: opts?.itemDescription || 'Top up your wallet balance',
    email: opts?.email || '',
  };

  const data: any = await callCallable('createPayFastPayment', payload);
...
export async function verifyPayFastTopUp(userId: string, paymentId: string) {
  const data: any = await callCallable('verifyPayFastPayment', { paymentId });
  return {
